...
func (ca *rolieCompletion) checkCompletion(p *processor, tlpe csaf.TLPLabel, entryName string) {
	...
	lv := ca.getLevel(tlpen)
	lv.addEntry(entryName)

	tlpfn := tlpLevel(ca.currentFeedLevel)
...
func (ca *rolieCompletion) getLevel(level int) *tlpLevels {
	lv := ca.levels[level]
	if lv == nil {
		lv = &tlpLevels{}
	}
...
func (ca *rolieCompletion) readySummary(entrylist []csaf.AdvisoryFile) {
	...
	level := tlpLevel(ca.currentFeedLevel)

	lv := ca.getLevel(level)

	if lv.checkSummary(entrylist) {
		lv.feeds = append(lv.feeds, ca.currentFeed)
	}
	/*
